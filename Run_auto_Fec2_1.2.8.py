...
def process_data(folder_path, selected_duration,UCL_UP_1,LCL_UP_1,UCL_UP_2,LCL_UP_2,UCL_LOW_1,LCL_LOW_1,UCL_LOW_2,LCL_LOW_2):
    ...
    for file_path in matching_files:
        ...
        print("data",data)
        required_columns = data[['Time', 'Oil Low number', 'Oil Low duration','Oil Up number','Oil Up duration']]
 
        filtered_columns = required_columns.dropna(subset=['Time'])

# Further filter out rows where "Oil Low duration" or "Oil Up duration" contain 'sec'
        filtered_columns = filtered_columns[~filtered_columns['Oil Low duration'].str.contains('sec', na=False)]
        filtered_columns = filtered_columns[~filtered_columns['Oil Up duration'].str.contains('sec', na=False)]
        
        

        
        last_row_index = filtered_columns.index[-1]
        print(f"Last processed row index: {last_row_index}")
        with open('last_row_indices.txt', 'w') as file:
            for file_path, last_row_index in last_row_indices.items():
                file.write(f"{file_path}: {last_row_index}\n")
        print("Last row indices have been saved to 'last_row_indices.txt'.")
