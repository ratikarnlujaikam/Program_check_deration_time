...
# Check if read_last_timestamp is greater than the current time
        ...
        for oil_low_number in oil_low_numbers:
            ...
            current_hour = pd.Timestamp.now().floor('H')  # ปัดเศษให้เป็นชั่วโมงที่ใกล้ที่สุด
            ...
            if hour_to_use not in hourly_avg.index:
                error = f"not have lest  hour {previous_hour}:00"
                submit_reset(error)
                root = tk.Tk()
                root.withdraw()  # Hide the main Tkinter window
                # messagebox.showerror("Error", f"not have lest  hour {previous_hour}:00")
...
# เปิดไฟล์ในโหมด append เพื่อเขียนข้อมูลเพิ่ม
    ...
    # เขียนข้อมูลจาก records ลงในไฟล์
        ...
        for oil_up_number in oil_up_numbers:
            ...
            print("hourly_avg for Oil Up number", oil_up_number, hourly_avg)
            
            latest_hour = hourly_avg.index[-1]
            latest_value = hourly_avg.iloc[-1]
            print("latest_hour for Oil Up:", latest_hour)
            print("latest_value for Oil Up:", latest_value)
            
            previous_hour = (latest_hour - 1) % 24 if latest_hour > 0 else latest_hour
            hour_to_use = previous_hour
            
            if hour_to_use not in hourly_avg.index:
                error = f"No recent hour found for {previous_hour}:00"
                submit_reset(error)
                root = tk.Tk()
                root.withdraw()
                return
            ...
            print("last_value for Oil Up:", last_value)
            print("max_hour_value for Oil Up:", max_hour_value)
            print("Diff for Oil Up:", Diff)
            
            filename = f'max_hour_value_Up{oil_up_number}.txt'
            with open(filename, 'w') as file:
                file.write(str(max_hour_value))
                
            time_and_duration = datafilter_ok[datafilter_ok['Oil Up number'] == oil_up_number][['Time', 'Oil Up duration', 'Barcode']]
...
def submit_reset(error):
    ...
    for file_path in matching_files:
        ...
        last_index -= 1
        next_index -= 1
        print("submit_reset_last_index", last_index)
        print("submit_reset_next_index", next_index)
        with open('last_index.txt', 'w') as file:
            file.write(f"{last_index},{next_index},{last_timestamp}")
            
    value=f"Reset Auto Skip Time ,{error}"
    Time = datetime.now()
    Time_format = Time.strftime('%H:%M:%S')
...
version_label = ttk.Label(frame, text="V.1.4.7")
