...
# Check if read_last_timestamp is greater than the current time
        ...
        for oil_low_number in oil_low_numbers:
            data_oil_up = data[data['Oil Low number'] == oil_low_number]
            ...
            time_and_duration = time_and_duration[time_and_duration['Oil Low duration'] != .0]
            ...
            for index, row in time_and_duration.iterrows():
                ...
                adjusted_duration = abs(float(oil_Low_duration_value) - max_hour_value)  # แปลงเป็น float ก่อนลบ max_hour_value
...
# เปิดไฟล์ในโหมด append เพื่อเขียนข้อมูลเพิ่ม
    ...
    # เขียนข้อมูลจาก records ลงในไฟล์
        ...
        for oil_up_number in oil_up_numbers:
            data_oil_up = data[data['Oil Up number'] == oil_up_number]
            print("data_oil_up",data_oil_up)
            barcode = str(data_oil_up['Barcode'].iloc[0]) 
            data_oil_up['Time'] = pd.to_datetime(data_oil_up['Time'], format='%H:%M:%S')
            ...
            time_and_duration = time_and_duration[time_and_duration['Oil Up duration'] != .0]
            print("time_and_duration",time_and_duration)
            for index, row in time_and_duration.iterrows():
                time_value = row['Time'].strftime('%H:%M:%S')
                Barcode_value = str(row['Barcode']).split('.')[0]
                ...
                adjusted_duration = abs(float(oil_up_duration_value) - max_hour_value)  # แปลงเป็น float ก่อนลบ max_hour_value
...
version_label = ttk.Label(frame, text="V.1.3.8")
