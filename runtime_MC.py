...
# # Display the last index
        print("Last Index:", last_index+3)

# Read the last stored index from the file
        with open('last_index.txt', 'r') as file:
            last_stored_index = int(file.read())
            print(last_stored_index)

# Increment the last stored index by 1
        next_index = last_stored_index + 1

# Read all data from the next index onward
        all_data_from_next_index = pd.read_csv(file_path, delimiter=';', skiprows=range(1, next_index))

# Display the data
        print("-------------------", all_data_from_next_index)

# Update and store the new last index in the file
        with open('last_index.txt', 'w') as file:
            file.write(str(last_index+1))
        ...
        for oil_low_number in oil_low_numbers:
            ...
            if oil_low_number == 1:
                UCL_H1 = float(UCL_LOW_1)
                LCL_H1 = float(LCL_LOW_1)
                if Diff > UCL_H1 :
                    message = (
                        f"Time : {formatted_time} \n"
                        f"Process : Oil Low duration \n"
                        f"Head : {oil_low_number} \n"
                        f"previous_hour: {int(previous_hour)}:00 hours \n"
                ...
                UCL_H2 = float(UCL_LOW_2)
                LCL_H2 = float(LCL_LOW_2)
                if Diff > UCL_H2:
                    message = (
                        f"Time : {formatted_time} \n"
                        f"Process : Oil Low duration \n"
...
# pyinstaller --onefile runtime_MC.py
