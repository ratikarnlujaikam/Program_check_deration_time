...
def show_warning(message):
    ...
    warning_window = tk.Toplevel()
    warning_window.title("SPC Alert")
    warning_window.geometry("400x280")
    warning_window.configure(bg="red")
...
def read_stop_time(stop_time_filename):
    try:
        ...
        if stop_time:
            try:
                # ลองแปลงรูปแบบเวลาเต็ม
                return datetime.strptime(stop_time, '%Y-%m-%d %H:%M:%S')
            except ValueError:
                try:
                    # ลองแปลงรูปแบบเวลาที่ไม่มีวันที่
                    return datetime.strptime(stop_time, '%H:%M:%S')
                except ValueError:
                    raise ValueError("time data '{}' does not match expected formats".format(stop_time))
        else:
            return None
...
def process_data(folder_path, selected_duration,UCL_UP_1,LCL_UP_1,UCL_UP_2,LCL_UP_2,UCL_LOW_1,LCL_LOW_1,UCL_LOW_2,LCL_LOW_2):
    ...
    for file_path in matching_files:
        ...
        print("stop_file_path******************************************************",stop_file_path)
...
# Check if read_last_timestamp is greater than the current time
        ...
        for oil_low_number in oil_low_numbers:
            ...
            stop_file_path = f'stop_time_{current_date_1}.txt'
            stop_time = read_stop_time(stop_file_path)
            print("stop_time")
            
            current_hour = datetime.now().replace(minute=0, second=0, microsecond=0)

            if stop_time is None:
                stop_time = current_hour - timedelta(hours=1)
                print("time stop is None, using previous hour:", stop_time)
            else:
                stop_time = stop_time.replace(minute=0, second=0, microsecond=0)
                print("using stop_time:", stop_time)
                
            if stop_time == current_hour:
                print("Stop time matches current hour.")
                hour_to_use = latest_hour  # Using latest hour's value
            else:
                print("Stop time does not match current hour, using previous hour.")
                hour_to_use = (latest_hour - 1) % 24  # Using previous hour's value
            if hour_to_use not in hourly_avg.index:
                raise KeyError(f"No data found for the given hour: {hour_to_use}")
...
version_label = ttk.Label(frame, text="V.1.2.6")
